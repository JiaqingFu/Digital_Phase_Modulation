...
% Last Modified by GUIDE v2.5 16-Jul-2014 15:57:17
...
if(get(handles.togglebutton1,'value'))
    y1 = get(handles.edit3,'string');
y1=str2Double(y1);
c=(randn(1,y1)>0);
end
 if(get(handles.togglebutton2,'value'))
     y2 =get(handles.edit4,'string');
y2 = (str2num(y2'))';
c=y2;
end
b=codec(c);
...
str1{value1}
str2{value2}
a=modulation(b,str1{value1});
out=demodulation(a,str1{value1},str2{value2},c);

space=num2str(out{2});
 space(find(isspace(space))) = []
class(space)
if(get(handles.togglebutton2,'value'))
 set(handles.edit5,'string',space);
end

set(handles.edit2,'string',num2str(out{1}(x)));
h1=figure('NumberTitle','off','Name','ÎóÂëÂÊ');
figure(h1);
plot(10*log10(out{1}));
hold on;
